...
`.trim(), m)
  let id = m.chat
  if (id in global.math) return conn.reply(m.chat, 'Still that question is not answered in the chat', global.math[id][0])
  ...
  global.math[id] = [
    ...
    math, 4,
    setTimeout(() => {
      if (global.math[id]) conn.reply(m.chat, `Time out!\nThe answer is  ${math.result}`, global.math[id][0])
